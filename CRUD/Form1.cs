...
namespace CSharpCrudGrid
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                //had to add fruit id to grid
                var fruit_id = row.Cells["colFruitID"].Value;
                var fruit_name = row.Cells["colFruitName"].Value;
                var fruit_is_yummy = row.Cells["colFruitIsYummy"].Value;
                var fruit_grows_on_id = row.Cells["colFruitGrownsOnID"].Value;

                //Will update all existing rows
                if (int.TryParse(fruit_id?.ToString(), out int fid))
                {
                    var updcmd = GetUpdateCommand();
                    updcmd.Connection = connection;
                    updcmd.Parameters["@FruitID"].Value = fruit_id;
                    updcmd.Parameters["@FruitName"].Value = fruit_name ?? DBNull.Value;
                    updcmd.Parameters["@FruitIsYummy"].Value = fruit_is_yummy ?? DBNull.Value;
                    updcmd.Parameters["@FruitGrowsOnID"].Value = fruit_grows_on_id ?? DBNull.Value;
                    updcmd.ExecuteNonQuery();
                }
                else //must be a new record
                {
                    var inscmd = GetInsertCommand();
                    inscmd.Connection = connection;
                    inscmd.Parameters["@FruitName"].Value = fruit_name ?? DBNull.Value;
                    inscmd.Parameters["@FruitIsYummy"].Value = fruit_is_yummy ?? DBNull.Value;
                    inscmd.Parameters["@FruitGrowsOnID"].Value = fruit_grows_on_id ?? DBNull.Value;
